...
openai_key_file = os.path.join(args.key_path, 'openai.key')
if os.path.isfile(openai_key_file):
    with open(openai_key_file, 'r') as f:
        os.environ["OPENAI_API_KEY"] = f.read().strip()
else:
    os.environ["OPENAI_API_KEY"] = "DUMMY_OPENAI_KEY"
    print("Warning: openai.key not found. Using dummy OPENAI_API_KEY.")

serpapi_key_file = os.path.join(args.key_path, 'serpapi.key')
if os.path.isfile(serpapi_key_file):
    with open(serpapi_key_file, 'r') as f:
        os.environ["SERPAPI_API_KEY"] = f.read().strip()
else:
    os.environ["SERPAPI_API_KEY"] = "DUMMY_SERPAPI_KEY"
    print("Warning: serpapi.key not found. Using dummy SERPAPI_API_KEY.")
