...
class PWS:
    def __init__(self, available_tools=["Wikipedia", "LLM"], fewshot="\n", planner_model="THUDM/agentlm-7b",
                 solver_model="THUDM/agentlm-7b"):
        self.workers = available_tools
        # print("self.workers0", self.workers)
        ...
        self.tool_token_unit_price = get_token_unit_price("THUDM/agentlm-7b")
    ...
    def run(self, input):
        ...
        # print("lens plans", len(self.plans) ,len(self.planner_evidences))
        # print("self.plans", self.plans)
        # print("self.planner_evidences", self.planner_evidences)
    ...
    def _parse_planner_evidences(self, response):
        ...
        for line in response.splitlines():
            if line.startswith("#") and line[1] == "E" and line[2].isdigit():
                ...
                # print("e, tool_call000000000000", e, tool_call)
                e, tool_call = e.strip(), tool_call.strip()
                # print("e, tool_call111111111111", e, tool_call)
                if len(e) == 3:
                   
                    evidences[e] = tool_call
                    # print("evidences[e] aftr", evidences[e])
                else:
        ...
        for e, tool_call in self.planner_evidences.items():
            ...
            if tool in self.workers:
                ...
                if tool == "Google":
                    ...
                    # print("self.tool_counter.get(LLM_token, 0)", self.tool_counter.get("LLM_token", 0)) 92
                    # print("2nd part", len(tool_input + self.worker_evidences[e]) // 4) 92
                    # print("self.worker_evidences[e]", self.worker_evidences[e]) The Cripple of Inishmaan.
                    # print("self.tool_counter[LLM_token] ", self.tool_counter["LLM_token"] ) 92
...
class PWS_Base(PWS):
    def __init__(self, fewshot=fewshots.HOTPOTQA_PWS_BASE, planner_model="THUDM/agentlm-7b",
                 solver_model="THUDM/agentlm-7b", available_tools=["Wikipedia", "LLM"]):
...
class PWS_Extra(PWS):
    def  __init__(self, fewshot=fewshots.HOTPOTQA_PWS_EXTRA, planner_model="THUDM/agentlm-7b",
                 solver_model="THUDM/agentlm-7b", available_tools=["Wikipedia", "Calculator", "LLM"]):
        super().__init__(available_tools=available_tools,
                         fewshot=fewshot,
                         planner_model=planner_model,
                         solver_model=solver_model)
